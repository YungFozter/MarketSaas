...
export const SubscriptionBlockedModal = ({ isOpen, onClose, onNavigateToSubscription }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      <div 
        className="relative w-full max-w-lg bg-white border border-slate-200/90 rounded-3xl shadow-2xl overflow-hidden p-6 sm:p-8 text-slate-900 animate-scale-up"
        role="dialog"
        aria-modal="true"
      >
        {/* Botón cerrar */}
        ...
        <div className="flex flex-col items-center text-center mb-6">
          ...
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider bg-amber-50 text-amber-700 border border-amber-200 mb-2">
        ...
        <form onSubmit={handleRedeem} className="space-y-4 mb-6">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 rounded-xl text-slate-900 font-mono text-center tracking-widest uppercase text-base placeholder:text-slate-400 outline-none transition-all"
                autoFocus
              />
            </div>
            {errorMessage && (
          ...
          <button
            ...
            className="w-full py-3 px-4 bg-emerald-600 hover:bg-emerald-500 active:scale-98 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold rounded-xl flex items-center justify-center gap-2 shadow-sm shadow-emerald-600/20 cursor-pointer transition-all"
          >
            {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-4 border-t border-slate-100 text-xs">
          <button
            ...
            className="w-full py-2.5 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 hover:text-slate-900 font-medium flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
          >
          ...
          <a
            ...
            className="w-full py-2.5 px-3 rounded-xl bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-700 font-medium flex items-center justify-center gap-1.5 transition-colors"
          >
