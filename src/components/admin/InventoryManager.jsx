...
export const InventoryManager = () => {
  ...
  const handleAddCategory = () => {
    ...
    if (activeCategories.includes(cleanCat)) {
      showToast('Esta categoría ya existe.', 'warning');
      return;
    }
    const updated = [...activeCategories, cleanCat];
    setStoreConfig({ ...storeConfig, categories: updated });
    setNewCategoryName('');
    showToast(`Categoría "${cleanCat}" agregada con éxito.`, 'success');
  };

  const handleRemoveCategory = (catName) => {
    if (activeCategories.length <= 1) {
      showToast('Debe haber al menos una categoría.', 'warning');
      return;
    }
  ...
  // Si se cierra el modal de edición, cerrar también el selector
  useEffect(() => {
    if (!editingProduct) {
      setIsCategoryDropdownOpen(false);
    }
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fadeIn">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-2xs flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Controla precios, costos de compra, existencias y catálogo en tiempo real.
          </p>
        </div>
      ...
      <div className="bg-white p-3.5 sm:p-4 rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-2xs flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
      ...
      {/* Barra Superior de Control de Paginación, Contador y Límite */}
      <div className="bg-white p-3 sm:p-4 rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-2xs flex flex-col lg:flex-row lg:items-center justify-between gap-3">
        {/* Lado Izquierdo: Contador de productos y Navegación de páginas */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-2.5 sm:gap-4">
          ...
          <div className="text-xs text-slate-500 font-medium whitespace-nowrap flex items-center gap-1.5 flex-wrap">
            ...
            <span>de <strong className="font-extrabold text-slate-800">{totalItems}</strong> productos</span>
          </div>

          {/* Botones de Paginación Superior: Anterior - Páginas - Siguiente */}
          {totalPages > 1 && (
            <div className="flex items-center gap-1 sm:gap-1.5 select-none overflow-x-auto py-0.5">
              ...
              <div className="flex items-center gap-1 shrink-0">
                {getPageNumbers(safePage, totalPages).map((page, idx) => {
                  if (page === '...') {
                    return (
                      <span key={`top-ellipsis-${idx}`} className="px-1.5 py-1 text-slate-400 font-bold text-xs">
                        ...
                      </span>
                    );
                  }
        ...
        {/* Lado Derecho: Selector de Límite (10, 25, 50, Todos) */}
      ...
      {selectedProductIds.length > 0 && (
        <div className="sticky top-[56px] sm:top-[68px] z-20 bg-rose-50/95 backdrop-blur-md border border-rose-200/90 p-3 sm:p-4 rounded-2xl sm:rounded-3xl flex flex-col sm:flex-row items-center justify-between gap-2.5 sm:gap-3 shadow-lg shadow-rose-950/10 transition-all animate-fadeIn">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end shrink-0">
            <button
              ...
              className="flex-1 sm:flex-none justify-center px-3.5 py-1.5 rounded-xl bg-white hover:bg-rose-100/50 text-slate-700 font-bold text-xs border border-rose-200 transition-colors cursor-pointer shadow-2xs"
            >
              Deseleccionar
            </button>
            <button
              ...
              className="flex-1 sm:flex-none justify-center px-4 py-1.5 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-extrabold text-xs shadow-md shadow-rose-600/20 transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
            >
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 select-none overflow-x-auto max-w-full py-0.5">
            ...
            <div className="flex items-center gap-1 shrink-0">
              {getPageNumbers(safePage, totalPages).map((page, idx) => {
                if (page === '...') {
                  return (
                    <span key={`bottom-ellipsis-${idx}`} className="px-1.5 py-1 text-slate-400 font-bold text-xs">
                      ...
                    </span>
                  );
                }
          ...
          >
            ...
            <form onSubmit={handleSaveForm} className="p-4 sm:p-8 space-y-4 overflow-y-auto flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {/* Categoría con lista desplegable scrolleable */}
                <div className="relative" ref={categoryDropdownRef}>
                  <label className="text-xs font-bold text-slate-700 block mb-1">Categoría</label>
                  <button
                    type="button"
                    onClick={() => setIsCategoryDropdownOpen(prev => !prev)}
                    className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs font-medium bg-white hover:border-slate-300 focus:outline-hidden focus:border-emerald-500 transition-colors text-left cursor-pointer shadow-2xs"
                  >
                    <span className={`truncate ${editingProduct.category ? 'text-slate-800 font-semibold' : 'text-slate-400'}`}>
                      {editingProduct.category || 'Seleccionar categoría...'}
                    </span>
                    <ChevronDown className={`w-4 h-4 text-slate-400 shrink-0 ml-2 transition-transform duration-200 ${isCategoryDropdownOpen ? 'rotate-180 text-emerald-600' : ''}`} />
                  </button>
      ...
      {isCategoryModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs animate-fadeIn">
          <div className="bg-white w-full max-w-lg rounded-3xl p-6 shadow-2xl border border-slate-200 space-y-5 animate-scaleUp">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-extrabold text-base text-slate-900">Gestión de Categorías de Productos</h3>
                  <p className="text-xs text-slate-500">Agrega o elimina categorías para organizar tu catálogo</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-700 block">
                Categorías Registradas ({activeCategories.length}):
              </label>
              <div className="flex flex-wrap gap-2 max-h-52 overflow-y-auto p-3 bg-slate-50 rounded-2xl border border-slate-200">
                {activeCategories.map((cat) => (
                  <div
                    key={cat}
                    className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-xl text-xs font-bold text-slate-800 border border-slate-200 shadow-2xs"
                  >
                    <span>{cat}</span>
            ...
            <div className="space-y-2 pt-1">
              <label className="text-xs font-bold text-slate-700 block">Agregar Nueva Categoría:</label>
              <div className="flex gap-2">
                <input
                  type="text"
                  placeholder="Ej. Mascotas, Panadería, Limpieza..."
                  ...
                  className="flex-1 px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs font-medium bg-slate-50 focus:bg-white focus:outline-hidden focus:border-emerald-500"
                />
                <button
                  type="button"
